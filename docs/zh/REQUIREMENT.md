...
| FR-11 | 嵌套软链接 | 在目录型软链接内部添加子级软链接，支持循环检测和深度限制 | P2 |

### 3.3 文件预览与编辑

| ID | 功能 | 描述 | 优先级 |
|----|------|------|--------|
| FR-12 | 纯文本文件预览与编辑 | 在 UI 中阅览和编辑纯文本源文件内容（如 .txt, .log, .json, .yaml, .py 等），支持编辑保存到源文件。操作对象为软链接指向的源文件。 | P0 |
| FR-13 | Markdown 渲染预览与编辑 | 使用 react-markdown + remark-gfm 渲染显示 Markdown 源文件（.md）。支持编辑模式与预览模式切换，编辑保存到源文件。操作对象为软链接指向的源文件。本地引用（图片/文档）按浏览器默认行为处理，不做路径重写。 | P0 |
| FR-14 | 二进制文件标识 | 对于非文本文件显示文件类型信息和大小，不尝试预览内容 | P2 |

### 3.4 备份执行

| ID | 功能 | 描述 | 优先级 |
|----|------|------|--------|
| FR-15 | 执行备份 | 手动触发一次备份操作：增量检测（mtime+size）→ 同步变更到 `data/` → `git add` → `git commit` →（可选）`git push`，支持进度展示 | P0 |
| FR-16 | 定时/自动备份 | 按配置的 cron 表达式定时自动执行备份，应用启动时自动加载已启用的仓库 | P1 |
| FR-17 | 备份历史查看 | 查看仓库的 Git 提交历史，支持分页 | P1 |
| FR-18 | 源文件回滚 | 选择历史提交版本，将源文件恢复到指定提交中的版本。支持全量回滚、按 symlink 选择性回滚、单文件恢复，回滚前可预览提交中文件内容 | P1 |
...
| ID | 功能 | 描述 | 优先级 |
|----|------|------|--------|
| FR-22 | 应用启动/停止 | 一键启动和停止整个应用，启动后自动打开浏览器。系统托盘图标提供"打开 UI"、"启动/停止服务器"、"退出"操作 | P0 |
| FR-23 | 本地文件浏览 | 安全浏览本地文件系统，用于选择软链接源文件和预览文件，限定在用户主目录和仓库根目录 | P0 |
| FR-24 | 健康检查 | 提供 `/health` 端点，返回应用运行状态、启动时间和版本信息 | P2 |
